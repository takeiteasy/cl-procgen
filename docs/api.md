...
| `(cave-grid->walls grid &key cell-size height floor ceiling normals tex-coords)` | Extrudes the solid cells of a `(height width)` bit grid (`cellular-automata`, `drunkards-walk`, `bsp-dungeon`, `maze`) into a `common-shapes:mesh` of vertical wall quads. Face-culled: a wall quad is only emitted where a solid cell borders an open cell or the grid edge. `floor`/`ceiling` (default `t`/`nil`) additionally cap every open cell at `z=0`/`z=height`. |
| `(marching-squares->mesh field &key iso cell-size height normals)` | Extracts iso-contours from a 2D single-float `field` (`noise-field-2d`, `diamond-square`) via marching squares, with linearly interpolated edge crossings. Returns two values: a `common-shapes:mesh` extruding each contour segment into a vertical wall quad (`z` 0 to `height`; pass `:height 0.0` for a flat/degenerate mesh), and the raw contour as a list of `#(v0 v1)` segments of `#(x y)` points — a zero-dependency polyline output. |
